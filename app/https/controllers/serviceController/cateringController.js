...
function cateringController() {
    return {
        register: async (req, res) => {
            ...
            const isExist = await ServiceModel.exists({ contact_number: req.body.brand.contact_number, serviceType: 'catering' });
            if (isExist) {
        ...
        allCatering: async (req, res) => {
            const document = await ServiceModel.find({ status: 'approved', serviceType: 'catering' });
            if (document.length < 1) {
                return res.status(403).json({ message: 'No Caterer found.' });
            }
